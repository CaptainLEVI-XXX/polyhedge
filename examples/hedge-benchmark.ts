...
const report = {
  ...
  assumptions: [
    ...
    'Quoted asks and fees; no historical data, estimated probabilities or annualized performance.',
...
const header = ['scenario', 'policy', 'budgetUsd', 'costUsd', 'worstUncoveredUsd', 'worstNetLossUsd',
  'maxExcessPayoutUsd', 'activeLegs', 'stressCostUsd', 'stressWorstUncoveredUsd', 'stressWorstNetLossUsd', 'completion'];
const csv = [header, ...rows.map(r => [r.scenario, r.policy, r.budgetUsd,
  ...
  r.stressed.worstUncoveredUsd, r.stressed.worstNetLossUsd, r.completion])]
